"""
train_gestures_fixed.py

Single-file replacement for the user's original script with these improvements:
 - file-level split (GroupShuffleSplit) to avoid leakage across train/test
 - smaller, regularized 1D-CNN using GlobalAveragePooling1D instead of Flatten
 - stronger on-the-fly augmentations (jitter, scaling, time-shift, random-crop-resize)
 - tf.data pipeline for efficient training and augmentation
 - centered pad/trim or interpolated resize for variable-length recordings
 - evaluation: confusion matrix + classification report
 - saves Keras model and float / int8 TFLite (with representative dataset)

Usage:
 - Place CSV files as imu_dataset/<label>_*.csv with channels: timestamp,ax,ay,az,gx,gy,gz
 - Run: python train_gestures_fixed.py

Note: keep an eye on number of CSVs per label (leave-one-file-out or GroupKFold is preferred
 when you have very few files per class). This script uses GroupShuffleSplit for a single holdout.
...
def resize_linear(arr, target=WINDOW_SIZE):
    # linear resample each channel to length target
    L, C = arr.shape
    if L == target:
        return arr
    xp = np.arange(L)
    xnew = np.linspace(0, L-1, target)
    out = np.zeros((target, C), dtype=arr.dtype)
    for c in range(C):
        out[:, c] = np.interp(xnew, xp, arr[:, c])
    return out
...
def random_crop_resize(x, crop_frac_range=(0.8, 1.0)):
    L = x.shape[0]
    ...
    return resize_linear(cropped, target=L)


def augment_window(x):
    # x: numpy array shape (WINDOW_SIZE, channels)
...
# ---------- load CSVs (file-level grouping) ----------

def load_windows_from_csvs(data_dir=DATA_DIR, window_size=WINDOW_SIZE, stride=STRIDE,
                           channels=CHANNELS, label_map=LABEL_MAP, accel_range=ACCEL_RANGE,
                           gyro_range=GYRO_RANGE, pad_mode='center'):
    ...
    for label in label_map:
        ...
        for fpath in files:
            ...
            arr[:, 0:3] /= accel_range
            arr[:, 3:6] /= gyro_range
            # if recording shorter than window, pad/resize to window_size
            N = arr.shape[0]
            if N < window_size:
                # prefer resize to avoid long zero padding; use center pad for minimal change
                arr2 = pad_or_trim_center(arr, target=window_size)
                wins = [arr2]
            else:
                wins = []
                for start in range(0, N - window_size + 1, stride):
                    wins.append(arr[start:start+window_size])
                # if last segment left out and was shorter than window, add center-trim last
...
def make_datasets(X_train, y_train, X_val, y_val, batch_size=BATCH_SIZE, training=True):
    # We use numpy_function to call augment_window (which is numpy-based). tf.numpy_function
    # loses shape info so we set shapes explicitly after mapping.
...
def build_model_small(window_size=WINDOW_SIZE, n_channels=len(CHANNELS), n_classes=len(LABEL_MAP)):
    ...
    x = Conv1D(16, kernel_size=7, padding='same', activation='relu', kernel_regularizer=l2(1e-4))(inp)
    x = BatchNormalization()(x)
    x = Conv1D(32, kernel_size=5, padding='same', activation='relu', kernel_regularizer=l2(1e-4))(x)
    x = BatchNormalization()(x)
    x = SpatialDropout1D(0.3)(x)
    x = Conv1D(32, kernel_size=3, padding='same', activation='relu', kernel_regularizer=l2(1e-4))(x)
    x = BatchNormalization()(x)
...
# ---------- representative generator for tflite ----------
...
def main():
    ...
    # file-level split: groups = file path for each window; ensure no file crosses train/test
    ...
    # save int8 quantized model (if possible)
    try:
